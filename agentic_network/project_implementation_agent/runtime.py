...
def start_project_implementation(
    ...
    *,
    ...
    if status != "BLOCKED":
        ...
        patches = _write_patch_set(run_dir, cleaned_objective)
...
def _write_patch_set(run_dir: Path, objective: str) -> list[str]:
    patches_dir = run_dir / "patches"
    patches_dir.mkdir(parents=True, exist_ok=True)
    patch_path = patches_dir / "patch_001.diff"
    patch_summary = run_dir / "46_generated_patch_set.md"
    artifacts = build_project_artifacts(objective, run_dir.name)
    changes: list[str] = []
    proposed_files: list[str] = []
    for generated_path, content in sorted(artifacts.items()):
        relative = _generated_relative_path(generated_path)
        proposed_files.append(relative)
    ...
    summary_lines = [
        "# Generated Patch Set",
        "",
        "This complete implementation patch is a proposal only and is not applied.",
        "",
        f"- {patch_path.name}: {len(proposed_files)} generated project files",
        "",
        "## Proposed Files",
        *[f"- `{path}`" for path in proposed_files],
    ]
    patch_summary.write_text("\n".join(summary_lines) + "\n", encoding="utf-8")
    return [str(patch_path)]


def _generated_relative_path(generated_path: str) -> str:
    path = PurePosixPath(generated_path.replace("\\", "/"))
    if path.is_absolute() or len(path.parts) < 2:
        raise ValueError(f"Generated template path is invalid: {generated_path}")
    relative = PurePosixPath(*path.parts[1:])
    if any(part in {"", ".", ".."} or part.lower() in PROTECTED_PARTS for part in relative.parts):
        raise ValueError(f"Generated template path is blocked: {generated_path}")
