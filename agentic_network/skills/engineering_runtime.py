...
def execute_engineering_action(
    ...
    if handler is None:
        raise ValueError("unsupported_engineering_skill")
    result = handler(action, payload, workspace)
...
def _migration_analysis(root: Path, config: Path | None) -> dict[str, object]:
    revisions = []
